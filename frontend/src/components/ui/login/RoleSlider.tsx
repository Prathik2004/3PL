...
const RoleSlider = ({ selectedRole, setSelectedRole }: RoleSliderProps) => {
  ...
  return (
    <div
      ...
      <div className="flex items-center justify-between w-full rounded-lg shadow-[0_1px_1px_rgba(0,0,0,0.05),0_4px_6px_rgba(34,42,53,0.04),0_24px_68px_rgba(47,48,55,0.05),0_2px_3px_rgba(0,0,0,0.04)] cursor-pointer mt-2">
        {roles.map((role, idx) => (
          <span
            onClick={() => {
            ...
            key={idx}
            className="relative flex-1 py-4 px-2 sm:p-7">
            {
              selectedRole === role && (
                <motion.span
                  layoutId="selected"
            ...
            <span className="relative z-10 flex flex-col items-center justify-center gap-2">
              ...
              <span className={` ${selectedRole === role ? "text-black" : "text-[#64748B]"} text-[12px]/[15px] sm:text-[14px]/[17px] font-semibold `}> {role} </span>
            </span>
          </span>
        ))}
      </div>
