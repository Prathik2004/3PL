  ...
  return (
    <div className="relative inline-flex items-center gap-1.5 cursor-pointer" ref={dropdownRef}>
      ...
      >
        ...
        <span className="text-[15px] font-bold text-slate-900 dark:text-slate-100">{selectedOption.label}</span>
      ...
      {isOpen && (
        <div className="absolute top-[calc(100%+8px)] left-0 w-[260px] rounded-[18px] border border-slate-100 bg-white p-2 shadow-[0_8px_30px_rgb(0,0,0,0.08)] dark:border-zinc-800 dark:bg-zinc-950 z-50">
          <div className="relative mb-2 px-1 pt-1">
            ...
            <input 
              ...
              className="w-full rounded-[10px] bg-[#f4f7f9] py-2 pl-10 pr-3 text-[14.5px] text-slate-900 outline-none dark:bg-zinc-900 dark:text-slate-100 placeholder:text-slate-500"
            />
          </div>
          <div className="h-px w-full bg-slate-100 dark:bg-zinc-800 mb-2 mt-1"></div>
          <div className="max-h-[280px] overflow-y-auto flex flex-col gap-0.5 px-1 pb-1 pt-1">
            ...
            {filteredOptions.map((opt) => (
              ...
              >
                <div className={`flex h-[18px] w-[18px] items-center justify-center rounded-full border-[1.5px] ${value === opt.value ? 'border-slate-800 bg-slate-800 dark:border-slate-200 dark:bg-slate-200' : 'border-slate-300 dark:border-zinc-700'}`}>
                   {value === opt.value && <div className="h-2 w-2 rounded-full bg-white dark:bg-zinc-900"></div>}
                </div>
