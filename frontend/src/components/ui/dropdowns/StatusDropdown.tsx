...
export const StatusDropdown = <T extends string>({ label, value, options, onChange, disabled }: FilterDropdownProps<T>) => {
  ...
  return (
    <div className="relative inline-flex items-center gap-1.5 cursor-pointer" ref={dropdownRef}>
      ...
      >
        <span className="text-[15px] font-medium text-slate-500 dark:text-slate-400">{label}:</span>
      ...
      {isOpen && (
        <div className="absolute top-[calc(100%+8px)] left-0 w-[240px] rounded-[18px] border border-slate-100 bg-white p-3 shadow-[0_8px_30px_rgb(0,0,0,0.08)] dark:border-zinc-800 dark:bg-zinc-950 z-50 flex flex-col gap-0.5">
          <div 
          ...
          >
            <div className={`flex h-[18px] w-[18px] items-center justify-center rounded-full border-[1.5px] ${value === 'all' ? 'border-slate-800 bg-slate-800 dark:border-slate-200 dark:bg-slate-200' : 'border-slate-300 dark:border-zinc-700'}`}>
              {value === 'all' && <div className="h-2 w-2 rounded-full bg-white dark:bg-zinc-900"></div>}
            </div>
          ...
          {options.map((opt) => {
            ...
            return (
              ...
              >
                <div className={`flex h-[18px] w-[18px] items-center justify-center rounded-full border-[1.5px] ${isActive ? 'border-slate-800 bg-slate-800 dark:border-slate-200 dark:bg-slate-200' : 'border-slate-300 dark:border-zinc-700'}`}>
                   {isActive && <div className="h-2 w-2 rounded-full bg-white dark:bg-zinc-900"></div>}
                </div>
                <span className={`text-[15px] ${isActive ? 'font-medium text-slate-900 dark:text-slate-100' : 'text-slate-600 dark:text-slate-400'}`}>
                  {opt.label}
                </span>
              </div>
            );
