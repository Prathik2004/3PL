...
export const ResponsiveFilterBar = () => {
 ...
 <SearchDropdown label="Client"
 options={clientOptions}
 value={filters.client}
 onChange={(v) => updateFilter('client', v)}
 disabled={isPending || isLoading}
 />
 <SearchDropdown label="Carrier"
 ...
 disabled={isPending}
 />
 <ExceptionsDropdown value={filters.exceptions}
 onChange={(v) => updateFilter('exceptions', v)}
 disabled={isPending}
 />
 {hasActiveFilters && (
 <button onClick={clearFilters}
 className="mt-2 sm:mt-0 text-[14px] font-medium text-slate-400 hover:text-slate-600 transition-colors sm:ml-1"
 >
 Clear All
 </button>
 )}
 ...
 className="w-full flex h-14 items-center justify-center rounded-xl bg-[#0F172A] text-[15px] font-bold text-white transition-colors hover:bg-slate-800"
 >
 Apply Filters
 </button>
 </div>
