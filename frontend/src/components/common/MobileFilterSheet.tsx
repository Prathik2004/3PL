...
export const MobileFilterSheet = ({ isOpen, onClose, children }: MobileFilterSheetProps) => {
 ...
 {/* Backdrop */}
 <div className={`absolute inset-0 bg-slate-900/40 backdrop-blur-[2px] transition-opacity duration-300 ease-out ${isOpen ?'opacity-100' :'opacity-0'}`}
 onClick={onClose}
 />

 {/* Sheet Content */}
 <div className={`relative z-10 w-full max-h-[85vh] transform flex flex-col rounded-t-[28px] bg-white shadow-2xl transition-transform duration-300 ease-out border-t border-slate-200 ${isOpen ?'translate-y-0' :'translate-y-full'}`}
 >
 {/* Drag handle */}
 <div className="flex w-full items-center justify-center pt-4 pb-2" onClick={onClose}>
 <div className="h-1.5 w-12 rounded-full bg-slate-200 cursor-grab" />
 </div>

 {/* Header */}
 <div className="flex items-center justify-between px-6 pb-4 border-b border-slate-100">
 <h2 className="text-xl font-bold tracking-tight text-slate-900">Filter Options</h2>
